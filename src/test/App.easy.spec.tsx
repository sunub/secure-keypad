...
const clickPadButton = async (id: number) => {
  const el = screen.findByTestId(id);
  return userEvent.click(await el);
}
const clickByNumOfCount = async (count: number) => {
  for (let i = 0; i < count; i++) {
    await clickPadButton(randomIndex());
  }
}
